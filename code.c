...
void ConvertTo8BitGrayscale(IMAGE *image) {
    int rowsize=(image->bmpih.biw*3+3)&~3; 
    int grayRowsize=(image->bmpih.biw+3)&~3;

    BYTE *newData=(BYTE *)malloc(grayRowsize*image->bmpih.bih);
    if (newData==NULL) {
    ...
    for (int y=0;y<image->bmpih.bih;y++) {
        for (int x=0;x<image->bmpih.biw;x++) {
            int index24=y*rowsize+x*3;
            int index8=y*grayRowsize+x;

            BYTE red=image->data[index24+2];
            BYTE green=image->data[index24+1];
            BYTE blue=image->data[index24];

            BYTE grayValue=(red+green+blue)/3;

            newData[index8]=grayValue;
        }
    }
    ...
    image->bmpih.biclused=256;
    image->bmpih.biclimp=256;
    image->bmpih.bisizeimage=grayRowsize*image->bmpih.bih;
    image->bmpfh.bfsize=sizeof(BMPFH)+sizeof(BMPIH)+256*sizeof(PALET)+grayRowsize*image->bmpih.bih;
    image->bmpfh.bfOffbits=sizeof(BMPFH)+sizeof(BMPIH)+256*sizeof(PALET);

    image->palet=(PALET *)malloc(sizeof(PALET)*256);
    if (image->palet==NULL) {
        printf("Not enough memory for palette");
        exit(1);
    }
...
int main() {
    ...
    ConvertTo8BitGrayscale(image);
    ImageWrite(image, "parrots_8bit.bmp");
